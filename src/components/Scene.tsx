...
import { useGLTF, useScroll, ScrollControls, Scroll, Bounds, Center } from '@react-three/drei';
import { EffectComposer, Bloom } from '@react-three/postprocessing';
import * as THREE from 'three';

function useViewportWidthAtZ(z = 0) {
  const { camera, viewport } = useThree();
  const v = viewport.getCurrentViewport(camera, new THREE.Vector3(0, 0, z));
  return v.width; // world units
}

function Model({ offsetX = 0, xvw, scale = 1, ...props }: any) {
  const { scene } = useGLTF('/Brain.glb');
  const groupRef = useRef<THREE.Group>(null!);
  const scroll = useScroll();
  const vwWidth = useViewportWidthAtZ(0);

  // x hedefini hesapla
  const targetX = useMemo(() => {
    if (xvw != null) return xvw * vwWidth; // yüzde tabanlı
    return offsetX; // world unit tabanlı
  }, [xvw, vwWidth, offsetX]);

  useEffect(() => {
    if (groupRef.current) groupRef.current.position.x = targetX;
  }, [targetX]);
